...
function check_employee($employee_id) {
    $conn = get_connection();
    $stmt = $conn->prepare("SELECT * FROM employee_maxbet WHERE employee_id = :employee_id");
...
function check_duplicate_transfer($bank_account_no, $amount) {
    $conn = get_connection();
    $stmt = $conn->prepare("SELECT * FROM tranferlog_maxbet_ptop 
...
function check_recent_transfer($bank_account_no, $amount) {
    $conn = get_connection();
    $stmt = $conn->prepare("
        SELECT * FROM tranferlog_maxbet_ptop 
...
function process_action($action, $data) {
    switch ($action) {
        ...
        case 'transferp2p':
            ...
            // Log transactions
            $stmt = $conn->prepare("INSERT INTO tranferlog_maxbet_ptop (amount, date_time, bankAccountNo) VALUES (:amount, :date_time, :bank_account_no)");
            $stmt->execute([
                'amount' => $data['amount'],
                'date_time' => $date_time,
            ...
            $stmt = $conn->prepare("INSERT INTO tranferlog_maxbet (phonenumber, amount, date_time, employee_name, employee_id) VALUES (:phonenumber, :amount, :date_time, :employee_name, :employee_id)");
            $stmt->execute([
        ...
        case 'transfer':
            try {
                ...
                try {
                    ...
                    $stmt = $conn->prepare("
                        INSERT INTO tranferlog_maxbet_ptop (
